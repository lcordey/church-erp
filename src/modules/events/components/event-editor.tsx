...
export function EventEditor({ canManage, event, setlists, users }: { canManage: boolean; event?: EventDetail; setlists: SetlistSummary[]; users: AssignableUser[] }) {
  ...
  if (!canManage && event) {
    return <main className="event-page"><div className="event-shell"><AppTopBar backHref="/events" backLabel="Retour aux événements" mode="public" /><article className="event-detail"><p className="eyebrow">Événement</p><h1>{event.title}</h1><p className="event-detail__date">{new Intl.DateTimeFormat("fr-FR", { timeZone: "Europe/Paris", dateStyle: "full", timeStyle: "short" }).format(new Date(event.startsAt))}</p>{event.notes ? <p className="event-detail__notes">{event.notes}</p> : null}{event.setlist ? <p><Link href={`/setlist/${event.setlist.id}/play`}>Ouvrir la setlist « {event.setlist.title} »</Link></p> : null}<section><h2>Équipe de service</h2>{event.assignments.length ? <ul>{event.assignments.map((assignment) => <li key={assignment.id}>{assignment.displayName}{assignment.role ? ` · ${assignment.role}` : ""}{assignment.userStatus === "disabled" ? " · compte désactivé" : ""}</li>)}</ul> : <p>Aucune personne affectée.</p>}</section></article></div></main>;
  }
