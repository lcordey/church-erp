...
- Android users may see an in-app PWA install button when the browser confirms
  installability, and installed sessions can prompt when a new service worker
  version is ready
...
2. Prepare the next MVP module only after the worship-team workflow is validated.
