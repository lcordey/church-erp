...
function renderSeedSql(officialSongs, gloriousSongs = []) {
  const allSongs = [...officialSongs, ...gloriousSongs, ...localSongs];
...
async function writeSeed(snapshot) {
  const gloriousSnapshot = await readJsonIfExists(gloriousSnapshotPath);
  await writeFile(
    seedPath,
    renderSeedSql(snapshot.songs, gloriousSnapshot?.songs ?? []),
    "utf8",
  );
}
